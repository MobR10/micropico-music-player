# algorithm to convert .mp3 files into json files with 2 arrays containing frequencies and durations for each frequency
# This algorithm was taken from this repository and changed a little bit since I was using a Raspberry PI Pico: 
...
def generate_json(frequencies, durations,wav_file):
    frequencies = [int(freq) for freq in frequencies]
    durations = [round(duration,2) for duration in durations]
    content = {"frequencies": frequencies, "durations": durations}
    with open(f"songs/{wav_file}.json", "w") as f:
        json.dump(content,f)
...
def process_mp3_file(file_path, tempo, max_size, method):
    ...
    grouped_melody, grouped_durations = group_notes(pitches, durations)
    ...
    generate_json(grouped_melody,grouped_durations,file_name)
